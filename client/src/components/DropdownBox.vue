<template>
    <div class="relative inline-block text-left">
        ...
        <button v-if="state.hidden" @click="state.hidden=false" class="z-10 fixed w-full h-full top-0 right-0 left-0 bottom-0"></button>
        <transition name="fade">
            <div v-if="state.hidden" class="z-20 origin-top-right absolute right-0 mt-2 w-56 rounded-md shadow-lg">
                <div class="rounded-md bg-white shadow-xs">
                    <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                        <a v-for="value in dropdown.values" :key="value"
                            @click="selected(dropdown.name, value)"
...
export default {
    ...
    setup(props, { emit }) {
        ...
        function selected(filter_name, value) {
            emit('update-selected', { id: props.identifier, selected: value })
